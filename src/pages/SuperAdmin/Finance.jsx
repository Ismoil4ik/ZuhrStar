...
export default function Finance() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [studentsData, salariesData, checksData, teachersData] = await Promise.allSettled([
          api.get('/students'),
          api.get('/salaries'),
          api.get('/checks'),
          api.get('/teachers')
        ...
        if (checksData.status === 'fulfilled') {
          ...
          const studentIds = [...new Set(checksArray.map(c => c.paid_student_id).filter(Boolean))];
  ...
  const totals = useMemo(() => {
    const totalSalaries = salaries.reduce((sum, s) => sum + n(s?.finalSalary || 0), 0);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 overflow-hidden">
      <div className="max-w-7xl mx-auto space-y-6">
        {/* Header */}
        <header className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 pt-2">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mb-1">
              Moliyaviy boshqaruv
            </h1>
            <p className="text-gray-600">O'quvchilar to'lovi, o'qituvchilar oyligi va tushumlar tahlili</p>
          </div>
          {updatedAt && (
            <span className="text-sm text-gray-600 bg-white border border-gray-200 px-3 py-1.5 rounded-lg self-start">
              Yangilandi: {updatedAt.toLocaleString('uz-UZ')}
            </span>
          )}
        </header>
        ...
        <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="rounded-xl bg-white border border-gray-200 p-5 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-gray-600">Jami o'quvchilar</span>
            ...
            <div className="mt-3 h-1.5 w-full bg-gray-200 rounded-full">
          ...
          <div className="rounded-xl bg-white border border-blue-200 p-5 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-blue-700">Faol o'quvchilar</span>
              <svg className="w-5 h-5 text-blue-500" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4Zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4Z" />
              </svg>
            </div>
            <div className="text-2xl font-bold text-blue-900">{studentStats.active}</div>
            <div className="mt-3 h-1.5 w-full bg-blue-200 rounded-full">
              <div className="h-1.5 bg-blue-600 rounded-full" style={{ width: `${studentStats.total > 0 ? (studentStats.active / studentStats.total) * 100 : 0}%` }} />
            </div>
          </div>

          <div className="rounded-xl bg-white border border-green-200 p-5 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-green-700">To'laganlar</span>
              <svg className="w-5 h-5 text-green-600" viewBox="0 0 24 24" fill="currentColor">
                <path d="M9 16.2 4.8 12l-1.4 1.4L9 19 21 7l-1.4-1.4z" />
              </svg>
            </div>
            <div className="text-2xl font-bold text-green-900">{studentStats.paid}</div>
            <div className="mt-3 h-1.5 w-full bg-green-200 rounded-full">
              <div className="h-1.5 bg-green-600 rounded-full" style={{ width: `${studentStats.total > 0 ? (studentStats.paid / studentStats.total) * 100 : 0}%` }} />
            </div>
          </div>

          <div className="rounded-xl bg-white border border-purple-200 p-5 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-purple-700">Umumiy balans</span>
            ...
            <div className="mt-3 h-1.5 w-full bg-purple-200 rounded-full">
              <div className="h-1.5 bg-purple-600 rounded-full w-[85%]" />
            </div>
          </div>
        </section>
        ...
        <section className="rounded-xl bg-white border border-gray-200 p-6 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
            <h3 className="text-lg font-bold text-gray-800">O'qituvchilar oyligi</h3>
            <span className="inline-flex items-center rounded-full bg-blue-50 border border-blue-200 px-3 py-1 text-sm font-semibold text-blue-800 self-start">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="rounded-xl bg-white border border-gray-200 p-6 shadow-sm">
            <h3 className="text-lg font-bold text-gray-800 mb-4">O'quvchilar holati</h3>
            <div className="h-80">
        ...
        <section className="rounded-xl bg-white border border-gray-200 p-6 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
            <h3 className="text-lg font-bold text-gray-800">Oylik tushum (checks)</h3>
            <span className="inline-flex items-center rounded-full bg-sky-50 border border-sky-200 px-3 py-1 text-sm font-semibold text-sky-800 self-start">
              Jami tushum: <b className="ml-1">{money(totals.totalRevenue)}</b>
            </span>
          </div>
          ...
          {/* Checks Table */}
          <div className="mt-6">
            <h4 className="text-base font-semibold text-gray-700 mb-3">Barcha to'lovlar</h4>
            ...
            <div className="block sm:hidden space-y-3">
              {checks.length > 0 ? (
                checks.slice(0, 5).map((check, idx) => {
                  ...
                  const studentName = student ? `${student.name || ''} ${student.surname || ''}`.trim() : 'Yuklanmoqda...';
                  const studentPhone = student?.student_phone || '—';

                  return (
                    <div key={check._id || idx} className="rounded-lg bg-gray-50 border border-gray-200 p-3">
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          <div className="text-sm font-bold text-gray-900 truncate max-w-[120px]">{studentName}</div>
                          <div className="text-xs text-gray-600">{studentPhone}</div>
                        </div>
                        <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold ${check.status === 'paid' || check.status === 'success'
                      ...
                      <div className="space-y-1 text-xs">
                        <div className="flex justify-between">
                          ...
                          <span className="font-bold text-gray-900">{money(check.amount)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-500">To'langan:</span>
                          <span className="font-medium text-gray-700">
                            {check.date_of_payment
                              ? new Date(check.date_of_payment).toLocaleDateString('uz-UZ')
                              : '—'
                            }
                          </span>
                        </div>
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr className="text-left text-xs font-semibold uppercase tracking-wider text-gray-600">
                    ...
                    <th className="px-4 py-3 hidden md:table-cell">O'quvchi ID</th>
                    <th className="px-4 py-3 hidden lg:table-cell">To'lov ID</th>
                ...
                <tbody className="divide-y divide-gray-200">
                  {checks.length > 0 ? (
                    checks.map((check, idx) => {
                      ...
                      return (
                        <tr key={check._id || idx} className="hover:bg-gray-50">
                          <td className="px-4 py-3 font-semibold text-gray-900 max-w-[120px] truncate">
                            {studentName}
                          </td>
                    ...
                    <tr>
                      <td colSpan={7} className="px-6 py-12 text-center">
                        <div className="flex flex-col items-center gap-3">
                          ...
                          <div className="text-gray-500 font-medium">To'lovlar topilmadi</div>
                        </div>
                      </td>
                    </tr>
                  )}
        ...
        <section className="rounded-xl bg-white border border-gray-200 p-6 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <h3 className="text-lg font-bold text-gray-800">O'qituvchilar batafsil ma'lumot</h3>
            <div className="flex flex-wrap gap-3">
              <span className="inline-flex items-center rounded-full bg-green-50 border border-green-200 px-3 py-1 text-sm font-semibold text-green-800">
                Jami oylik: <b className="ml-1">{money(totals.totalSalaries)}</b>
              </span>
              <span className="inline-flex items-center rounded-full bg-blue-50 border border-blue-200 px-3 py-1 text-sm font-semibold text-blue-800">
                O'qituvchilar: <b className="ml-1">{salaries.length}</b>
              </span>
              <button
                onClick={() => setShowSalaryForm(true)}
                className="inline-flex items-center gap-2 rounded-lg bg-blue-600 hover:bg-blue-700 px-4 py-2 text-white font-semibold transition-colors shadow hover:shadow-md"
              >
          ...
          <div className="block sm:hidden space-y-3">
            {salaries.length > 0 ? (
              salaries.slice(0, 5).map((salary, idx) => (
                <div key={salary._id || idx} className="rounded-lg bg-gray-50 border border-gray-200 p-3">
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <h4 className="font-semibold text-gray-800 text-sm truncate max-w-[140px]">{salary?.teacher?.fullName || "Noma'lum"}</h4>
                      <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium mt-1 ${salary?.teacher?.role === 'HeadMentor' || salary?.teacher?.role === 'head_mentor'
                        ? 'bg-purple-100 text-purple-700'
                        : salary?.teacher?.role === 'Mentor' || salary?.teacher?.role === 'mentor'
                    ...
                    <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold ${salary?.status === 'paid'
                      ? 'bg-green-100 text-green-800'
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div>
                      ...
                      <p className="font-medium text-gray-700">{salary?.month || '?'}/{salary?.year || '?'}</p>
                    </div>
                    <div>
                      <span className="text-gray-500">Bazaviy:</span>
                      <p className="font-medium text-gray-700">{money(salary?.baseSalary || 0)}</p>
                    </div>
                    <div>
                      <span className="text-gray-500">Bonus:</span>
                      <p className="font-medium text-green-700">{money(salary?.bonuses || 0)}</p>
                    </div>
                    <div>
                      <span className="text-gray-500">Chegirma:</span>
                      <p className="font-medium text-red-700">{money(salary?.deductions || 0)}</p>
                    </div>
                    <div className="col-span-2">
                      <span className="text-gray-500">Jami oylik:</span>
                      <p className="font-bold text-gray-900 text-sm">{money(salary?.finalSalary || 0)}</p>
                    </div>
                    {salary?.notes && (
                      <div className="col-span-2">
                        <span className="text-gray-500">Izoh:</span>
                        <p className="font-medium text-gray-700 text-xs truncate">{salary.notes}</p>
                      </div>
                    )}
                  </div>
                </div>
            ...
            <table className="min-w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr className="text-left text-xs font-semibold uppercase tracking-wider text-gray-600">
                  ...
                  <th className="px-4 py-3">Oy/Yil</th>
                  <th className="px-4 py-3 text-right">Bazaviy</th>
                  <th className="px-4 py-3 text-right">Bonus</th>
                  <th className="px-4 py-3 text-right">Chegirma</th>
                  <th className="px-4 py-3 text-right">Jami oylik</th>
                  <th className="px-4 py-3">Status</th>
                  <th className="px-4 py-3 hidden xl:table-cell">Izoh</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                {salaries.length > 0 ? (
                  salaries.map((salary, idx) => (
                    <tr key={salary._id || idx} className="hover:bg-gray-50">
                      <td className="px-4 py-3 text-gray-600 font-medium">{idx + 1}</td>
                      <td className="px-4 py-3 font-semibold text-gray-800 max-w-[150px] truncate">
                        {salary?.teacher?.fullName || "Noma'lum"}
                      </td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${salary?.teacher?.role === 'HeadMentor' || salary?.teacher?.role === 'head_mentor'
                          ? 'bg-purple-100 text-purple-700'
                          : salary?.teacher?.role === 'Mentor' || salary?.teacher?.role === 'mentor'
                            ? 'bg-blue-100 text-blue-700'
                            : 'bg-gray-100 text-gray-700'
                  ...
                  <tr>
                    <td colSpan={10} className="px-6 py-12 text-center">
                      <div className="flex flex-col items-center gap-3">
                        <div className="text-4xl">👨‍🏫</div>
                        <div className="text-gray-500 font-medium">Oylik ma'lumotlari topilmadi</div>
                      </div>
                    </td>
                  </tr>
                )}
      ...
      {showSalaryForm && (
        <>
          ...
          } bg-white shadow-2xl z-50 transform transition-transform duration-300 ease-in-out overflow-y-auto`}>
            <div className="p-6">
              {/* Header */}
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-bold text-gray-900">Yangi oylik qo'shish</h2>
                <button
                  onClick={() => setShowSalaryForm(false)}
              ...
              <form onSubmit={handleCreateSalary} className="space-y-5">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => handleSalaryFormChange('teacherId', e.target.value)}
                    required
                  ...
                  >
                    ...
                    {teachers.map(teacher => (
                      <option key={teacher._id} value={teacher._id}>
                        {teacher.fullName || teacher.name} - {teacher.role || 'Teacher'}
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => handleSalaryFormChange('year', e.target.value)}
                      required
                      min="2020"
                      max="2100"
                      className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => handleSalaryFormChange('bonuses', e.target.value)}
                    min="0"
                    placeholder="0"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => handleSalaryFormChange('deductions', e.target.value)}
                    min="0"
                    placeholder="0"
                ...
                {salaryForm.baseSalary && (
                  <div className="rounded-lg bg-gray-50 border border-gray-200 p-4">
                    <div className="text-sm text-gray-600 mb-2">Jami oylik:</div>
